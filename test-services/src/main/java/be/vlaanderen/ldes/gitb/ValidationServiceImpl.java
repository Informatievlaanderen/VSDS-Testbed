...
public class ValidationServiceImpl implements ValidationService {
    ...
    public ValidationResponse validate(ValidateRequest validateRequest) {
         ...
         */
        LOG.info("Requested validation from test session [{}]", validateRequest.getSessionId());
        // Get the expected inputs.
        var content = Utils.getRequiredString(validateRequest.getInput(), "content");
        var contentType = Utils.getRequiredString(validateRequest.getInput(), "contentType");
        // To illustrate the logging capabilities we will use this class to add log statements to the test session's log.
        var logger = new ValidationServiceLogger(validateRequest.getSessionId(), getReplyToAddressFromHeaders(wsContext).orElse(null));
        // Carry out the validation.
        var errorMessages = relationTimestampValidationHandler.validate(content, contentType, logger);
